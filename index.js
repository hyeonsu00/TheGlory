...
//db에서 값꺼낸거 정리 할 배열선언
const coretask_value = Array.from({ length: 5 }, () =>
  Array.from({ length: 5 }, () => [])
); //task별로 정렬시키기
const coretask_result = Array.from({ length: 5 }, () =>
  Array.from({ length: 5 }, () => [])
...
app.post('/upload', upload.single('userfile'), (req, res) => {
  // 새로운 파일을 위해 기존 기록 제거
  connection.query('DELETE FROM my_table');
  ...
  connection.query(
    ...
    (error, results, fields) => {
      ...
      // 각 task마다 core1~5까지의 값 묶어서 넣기 이 부분 너무 어려움!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      for (let j = 0; j < 10; j++) {
...
// 이 부분까지 inputFile.txt 파일을 넣고 MySQL DB에 넣기 위한 과정!!!
